...
def plot_box(onBox):
    ...
    label = label + "  frame:  " + str(personUndetectedCounter)
...
personDetectedOld = False
personDetectedNow = False
personUndetectedCounter = 0 # counts number of frame without person beeing detected
...
while True:
    ...
    frame = imutils.resize(frame, width=scale*400)
    # Espelha a imagem
    ...
    # count delta Time for the interaction
    personDetectedOld = personDetectedNow
    if (len(locs) > 0):
        personDetectedNow = True
        personUndetectedCounter = 0
        
    else:
        personUndetectedCounter += 1
        if (personUndetectedCounter > 4):
            personDetectedNow = False
    
    if (personDetectedOld == False and personDetectedNow == True):
        startTime = datetime.now()
    
    if (personDetectedNow == False and personDetectedOld == True):
        endTime = datetime.now()
        print(endTime - startTime)
        arquivo = open('timeTesting.txt', 'a')
        arquivo.write(
            "StartTime: " + (str(startTime) +
            "\nEndTime: " + str(endTime) + 
            "\nDeltaTime: " + str(endTime-startTime) +
            '\n\n'))
        arquivo.close()
    ...
    if (onBoxCounter % 50 == 0) and (face_on_box) and (len(faces) != 0):
